    ...
    NavigationView {
      VStack {
        List {
          Section {
            ...
            ForEach(data.reminders) { reminder in
              NavigationLink(
                destination: ReminderEdit(reminder: reminder.mutable, isNew: false) { r in
          ...
          Section {
            Label(
              data.upcomings.isEmpty ? "No upcoming reminders" : "Upcoming reminders",
        ...
        if notificationsDenied {
          Button(
            "Notifications are denied. Please enable notifications for this app in system settings."
          ) {
    ...
    .onChange(of: scenePhase) { phase in
      ...
      if phase == .active {
        Task {
          await data.refresh()
          notificationsDenied =
            (await Notifications.current.settings().authorizationStatus) == .denied
        }
      }
    }
  }
